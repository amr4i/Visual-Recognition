...
for batch_idx in tqdm(range(args.maxIter)):
    # forwarding
    optimizer.zero_grad()
    output = model( data )[ 0 ]
...
if not args.visualize:
    ...
    im_target_rgb = np.array([label_colours[ c % 100 ] for c in im_target])
    im_target_rgb = im_target_rgb.reshape( im.shape ).astype( np.uint8 )
cv2.imwrite( "results_backprop/"+args.input.split("/")[-1].split(".")[0]+".png", im_target_rgb )
...
# plt.show()
plt.savefig("results_backprop/" + args.input.split("/")[-1].split(".")[0] + '_TrainingLoss.png')
